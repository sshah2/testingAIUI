...
def apply_program():
    return {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "programappliedfor": {"type": "string"},
                "case_situation": {"type": "string"}
            },
            "required": ["programappliedfor", "case_situation"],
        },
        "description": """This function helps an applicant for an application for a specific program and the specific 
        user-provided household situation. The only programs that can be applied for are TANF, SNAP or Medicaid.
        You should not proceed if the program or the specific household situation has not been provided"""
    }
...
def main():
    ...
    if prompt:
        ...
        if return_value == "applyProgram":
            ...
            st.session_state["intake_scenario"] = arguments_array[1]
...
def display_app_details_in_tab(app_details_param):
    ...
    with st.sidebar:
        ...
        st.chat_message("assistant").write("Program Applied For: " + getattr(app_details_param, "programappliedfor"))
